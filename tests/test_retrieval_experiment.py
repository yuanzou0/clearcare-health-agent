...
def test_retrieval_experiment_compares_same_frozen_cases():
    ...
    assert report.relevant_case_count == 24
    assert report.no_hit_case_count == 31
    results = {result.strategy: result for result in report.strategies}
    assert results["keyword"].metrics["recall_at_k"] == 0.625
    assert results["bm25"].metrics["recall_at_k"] > 0.625
    assert (
