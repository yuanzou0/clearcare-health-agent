...
def test_health_mvp_dataset_is_versioned_privacy_safe_and_has_83_cases():
...
def test_label_review_distinguishes_consistency_from_expert_validation():
    ...
    assert report.human_review_pending_count == 83
...
def test_evaluation_cli_writes_json_and_markdown_reports(tmp_path):
    ...
    assert payload["case_count"] == 83
    assert "Evaluated 83 cases" in completed.stdout
